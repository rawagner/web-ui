...
import { ANNOTATION_CLONE_REQUEST } from '../constants';
...
export const cloneVm = (k8sCreate, vm, newName, newNamespace, newDescription, startVm, clonedDisks) => {
  ...
  clonedDisks.forEach(disk => {
    const oldVolumeName = disk.volumeName;
    const oldVolume = getVolumes(vm).find(volume => volume.name === oldVolumeName);
    const newPvcName = getName(disk.result);

    if (oldVolume.persistentVolumeClaim) {
      delete oldVolume.persistentVolumeClaim;
    }

    oldVolume.dataVolume = {
      name: newPvcName,
    };
  });
  ...
  const pvcToClone = {
    ...
    clone: {
      ...
      metadata: {
        ...
        annotations: {
          [ANNOTATION_CLONE_REQUEST]: `${getNamespace(pvc)}/${pvcName}`,
        },
      },
      spec: {
        source: {
